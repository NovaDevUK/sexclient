...
public class Glide extends Module{
    ...
    public PenisESP() {
        super("PenisESP", Keyboard.KEY_NONE, Category.Random);

    }

    @Override
    public void setup() {
        Gorda.instance.settingsManager.rSetting(new Setting("Animation", this, true));
    }
    ...
    public void onRender(final EventRender3D event) {
        for (final Object o : PenisESP.mc.world.loadedEntityList) {
            ...
            if (Gorda.instance.settingsManager.getSettingByName("Animation").getValBoolean()) {
                ++this.amount;
                if (this.amount > 25) {
                    ++this.spin;
                    if (this.spin > 50.0f) {
